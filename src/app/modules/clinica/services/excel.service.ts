...
import { LOGO, MINERVA } from '../constants/logo';
...
export class ExcelService {
  ...
  async downloadExcel(dataExcel: IConsultaExcelTabla) {
    ...
    await this.crearTablaMedicos(dataExcel.tablaMedico);
  ...
  private async crearTablaMedicos(dataMedicoTabla: ITablaMedico[]) {
    console.log('medicos: ', dataMedicoTabla);

    const sheet = this.workbook.addWorksheet("MÉDICOS");
    ...
    const logoMinerva = this.workbook.addImage({
      base64: MINERVA,
      extension: 'png'
    })

    const positionMinerva: ImagePosition = {
      tl: { col: 11, row: 1.2 },
      ext: { width: 128, height: 128 },
    }
    ...
    sheet.addImage(logoID, position);
    sheet.addImage(logoMinerva, positionMinerva);
    ...
    sheet.mergeCells("F5", "J5");
    const subtitulo = sheet.getCell("F5");
    subtitulo.value = "Reporte de Médicos";
    ...
    const filasInsertar = sheet.getRows(10, dataMedicoTabla.length)!;
    for (let index = 0; index < filasInsertar.length; index++) {
      const itemData = dataMedicoTabla[index];
      const row = filasInsertar[index];
      row.values = [
        "",
        "",
        ...
        `${index + 1}`,
        `${itemData.duiMedico}`,
        `${itemData.nombreCompletoMedico}`,
        `${itemData.correoMedico}`,
        `${itemData.telefonoMedico}`,
        `${itemData.direccionMedico}`,
      ];
